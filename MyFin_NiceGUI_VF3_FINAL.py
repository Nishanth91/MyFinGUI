    ...
    def content() -> None:
        ...
        for i in range(len(df)):
            ...
            if not tx.empty:
                ...
                # Normalize card/account names for matching:
                # "CanadianTire Mastercard - Grey" should match "CT Mastercard - Grey"
                def _norm_card(s: str) -> str:
                    s = s.lower().strip()
                    s = s.replace('canadian tire', 'ct').replace('canadiantire', 'ct')
                    # collapse whitespace
                    s = ' '.join(s.split())
                    return s

                _card_norm = _norm_card(card_key)
                _tx_accts = txu.get('account','').astype(str).str.strip()
                _acct_match = _tx_accts.apply(lambda a: _norm_card(a) == _card_norm)
            ...
            try:
                ...
                hist = hist[hist.get('account','').astype(str).str.strip().apply(lambda a: _norm_card(a) == _card_norm)].copy()
